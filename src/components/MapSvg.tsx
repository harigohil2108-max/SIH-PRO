...
interface Props {
  ...
  locationPickerDraggable?: boolean;
  interactive?: boolean;
...
export default function MapSvg({ mode = "markers", height = 420, showControls = true, showLocationPicker = false, locationPickerDraggable = true, interactive = true, allowMapClick = true, location, onLocationChange }: Props) {
  ...
  const initialCenter: [number, number] = location?.longitude != null && location?.latitude != null
    ? [location.longitude, location.latitude]
    : CENTER;

  useEffect(() => {
    if (!containerRef.current || !MAPTILER_KEY) return;
    const map = new maplibregl.Map({ container: containerRef.current, style: mapStyle(), center: initialCenter, zoom: 13, attributionControl: true });
    mapRef.current = map;
    if (!interactive) {
      map.dragPan.disable();
    ...
    map.on("load", () => {
      ...
      map.on("click", (event) => {
        if (!interactive || !showLocationPicker || !allowMapClick) return;
  ...
  }, [showControls, allowMapClick, interactive]);
  ...
  useEffect(() => {
    ...
    pickerRef.current = new maplibregl.Marker({ color: "#2563eb", draggable: locationPickerDraggable }).setLngLat([longitude, latitude]).addTo(mapRef.current);
    if (locationPickerDraggable) pickerRef.current.on("dragend", () => {
  ...
  }, [showLocationPicker, locationPickerDraggable]);
