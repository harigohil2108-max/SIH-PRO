...
export const addGrievanceEvidence = async (req, res) => {
  try {
    ...
    if (!name || !["IMAGE", "VIDEO", "DOCUMENT"].includes(type)) {
      return res.status(400).json({
        success: false,
    ...
    grievance.evidence.push({ url: String(name).trim(), type });
    grievance.timeline.push({
      status: grievance.status,
      message: `New ${type.toLowerCase()} evidence added: ${String(name).trim()}`,
      actor: req.user._id,
    });
    await grievance.save();
...
export const confirmGrievanceResolution = async (req, res) => {
  try {
    ...
    return res.json({ success: true, message: "Resolution accepted successfully", grievance });
  } catch (error) {
    console.error("Confirm grievance resolution error:", error);
    return res.status(500).json({ success: false, message: "Server error while confirming resolution" });
  }
};
