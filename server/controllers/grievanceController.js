...
export const assignGrievance =
  ...
  // ============================================================
  // ADD GRIEVANCE MESSAGE
  // POST /api/grievances/:id/messages
  // ============================================================

  export const addGrievanceMessage = async (req, res) => {
    try {
      ...
      if (req.user.role === "CITIZEN" && grievance.citizen.toString() !== req.user._id.toString()) {
        return res.status(403).json({
          success: false,
          message: "You can only message your own grievances",
        });
      }

      const actor = req.user.role === "CITIZEN" ? "Citizen" : "Officer";
      grievance.timeline.push({
      ...
      const recipient = req.user.role === "CITIZEN"
        ? grievance.assignedOfficer
        : grievance.citizen;
      if (recipient) {
        await Notification.create({
          user: recipient,
          title: "New Grievance Message",
          message: `A new message was added to grievance ${grievance.grievanceId}.`,
          type: "SYSTEM",
          relatedGrievance: grievance._id,
        });
      }
      ...
      console.error("Add grievance message error:", error);
      res.status(500).json({
        success: false,
  ...
  // ============================================================
  // ADD GRIEVANCE EVIDENCE
  // POST /api/grievances/:id/evidence
  // ============================================================

  export const addGrievanceEvidence = async (req, res) => {
    try {
      const name = String(req.body.name || "").trim();
      const type = String(req.body.type || "DOCUMENT").toUpperCase();
      const allowedTypes = ["IMAGE", "VIDEO", "DOCUMENT"];

      if (!name || !allowedTypes.includes(type)) {
        return res.status(400).json({
          success: false,
          message: "A valid evidence file name and type are required",
        });
      }
      ...
      if (req.user.role === "CITIZEN" && grievance.citizen.toString() !== req.user._id.toString()) {
        return res.status(403).json({
          success: false,
          message: "You can only add evidence to your own grievances",
        });
      }

      grievance.evidence.push({ url: name, type });
      grievance.timeline.push({
        status: grievance.status,
        message: `New ${type.toLowerCase()} evidence added: ${name}`,
        actor: req.user._id,
      });
      await grievance.save();
      ...
      if (recipient) {
        await Notification.create({
          user: recipient,
          title: "New Grievance Evidence",
          message: `New evidence was added to grievance ${grievance.grievanceId}.`,
          type: "SYSTEM",
          relatedGrievance: grievance._id,
        });
      }
      ...
      console.error("Add grievance evidence error:", error);
      res.status(500).json({ success: false, message: "Server error while adding evidence" });
    }
  };
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(403).json({
          success: false,
          message: "You can only escalate your own grievances",
        });
      }
...
export const confirmGrievanceResolution = async (req, res) => {
  try {
    const grievance = await Grievance.findById(req.params.id);
    if (!grievance) {
      return res.status(404).json({ success: false, message: "Grievance not found" });
    ...
    if (req.user.role === "CITIZEN" && grievance.citizen.toString() !== req.user._id.toString()) {
      return res.status(403).json({ success: false, message: "You can only confirm your own grievances" });
    }
    ...
    if (grievance.assignedOfficer) {
      await Notification.create({
        user: grievance.assignedOfficer,
        title: "Resolution Accepted",
        message: `The citizen accepted the resolution for grievance ${grievance.grievanceId}.`,
        type: "STATUS_UPDATE",
        relatedGrievance: grievance._id,
      });
    }

    res.json({ success: true, message: "Resolution accepted successfully", grievance });
  } catch (error) {
    console.error("Confirm grievance resolution error:", error);
    res.status(500).json({ success: false, message: "Server error while confirming resolution" });
  }
};